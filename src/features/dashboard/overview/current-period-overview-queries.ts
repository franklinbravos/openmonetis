...
/**
 * O que conta como movimento em relatório.
 *
 * Fora ficam os lançamentos que o próprio app gera para fechar saldo — saldo
 * inicial, pagamento de fatura e ajuste de saldo: contá-los inventa receita ou
 * despesa que ninguém movimentou.
 */
const shouldIncludeWithoutAutoGenerated = (row: {
	name: string;
	note: string | null | undefined;
}) =>
	!isInitialBalanceNote(row.note) &&
	!isAutoInvoiceNote(row.note) &&
	!isAccountBalanceAdjustmentName(row.name);
...
const shouldIncludeInPaymentStatus = (row: CurrentPeriodTransactionRow) => {
	...
	// Andaime de saldo não é receita nem despesa.
...
const shouldIncludeNamedItem = (name: string) => {
	...
	if (isAccountBalanceAdjustmentName(name)) {
		return false;
	}
...
const buildExpenseBreakdown = (
	...
	for (const row of rows) {
		if (
			row.transactionType !== TRANSACTION_TYPE_EXPENSE ||
			!shouldIncludeWithoutAutoGenerated(row)
		) {
			continue;
		}

...
- Importação de extrato: o ajuste de saldo **recua sozinho** conforme os meses anteriores vão sendo importados. O ajuste é andaime, não lançamento — ele existe só para impor a abertura que o banco declara enquanto os meses de trás ainda não entraram —, e por isso mora sempre no último dia do mês anterior ao extrato. Importar agosto o deixa em 31/07; importar julho depois o encontra no próprio mês do extrato, empurra-o para 30/06 e recalcula o valor pela abertura de julho. Julho e agosto passam a ser movimento real e o andaime encolhe. Repetindo mês a mês para trás, ele some quando o primeiro extrato entrar. A regra virou `resolveBalanceAdjustmentPlacement`, com teste da cadeia (agosto → julho → junho → maio), da virada de ano e do fevereiro bissexto — inclusive do invariante que a sustenta: o ajuste que um mês cria cai exatamente no mês que a importação seguinte vai relocar, senão cada importação criaria um ajuste novo e eles se somariam.
- Importação de extrato: o bloco **Saldo da conta** passa a mostrar a conferência inteira, em três partes — *no extrato* (saldo inicial, entradas, saídas, saldo final), *abertura do mês* (o saldo que o cadastro tem na véspera e o ajuste que o extrato impõe) e *movimento do mês* (líquido do arquivo contra líquido do cadastro). Antes o ajuste aparecia sozinho: um "+R$ 42.575,19" sem o saldo de onde ele sai não é conferível, é um susto. E quando o saldo final não fecha, a diferença passa a ser **decomposta até fechar**, em duas parcelas que apontam para consertos diferentes: lançamentos arquivados no mês mas **datados em outro mês** (o período do arquivo carimbado em toda linha) e lançamentos do mês que **não estão no extrato** (tipicamente perna de transferência sintetizada ao importar a conta do outro lado, sem id do banco). No extrato real de agosto/2026 da conta Nubank as duas somam R$ 2.669,23 + R$ 1.100,00 = exatamente os R$ 3.769,23 de diferença. Só o que está **realizado** entra na conta, que é o critério do saldo — contar o não realizado fazia a explicação não fechar com a própria diferença.
- Importação de extrato: a linha **conferida** passa a dizer que é pagamento de fatura, de qual cartão e de qual mês. Antes ela mostrava só a descrição, o selo "Conferido" e o tipo "Despesa" — visualmente idêntica a uma despesa comum —, então não havia como saber se o pagamento estava vinculado à fatura certa, que é justamente o que se confere antes de confirmar. O selo lê o vínculo **do lançamento já cadastrado** (a anotação `AUTO_FATURA`), não o palpite da importação: numa linha conferida nada é gravado, e o que importa é o que está no banco. Cartão e mês vêm sempre em par — cartão registrado com mês adivinhado seria uma fatura que não existe. Quando o cadastro casado não aponta fatura nenhuma, o selo fica âmbar e diz isso: é uma despesa comum disfarçada de "conferido", e o tooltip informa qual fatura o valor e a data indicam. A coluna **Tipo** também deixa de mostrar "Despesa" nessas linhas, passando a mostrar "Pgto. fatura" ou "Transferência".
- Importação de extrato: reconhecimento do **débito automático da fatura**, como o Inter escreve. O extrato do Inter não diz "pagamento de fatura" em lugar nenhum — diz `Pagamento efetuado: "Debito Automatico Fatura Cartao Inter"` —, então nenhum dos padrões existentes o alcançava e o pagamento entrava como despesa comum, sem liquidar fatura.
- Importação de extrato: o **pagamento de fatura se vincula sozinho ao cartão, pelo valor**. O extrato do Nubank descreve a linha só como "Pagamento de fatura" — sem nome de cartão e sem período —, então o palpite por nome não tinha o que casar e o usuário preenchia os dois à mão em toda importação. O valor identifica a fatura: R$ 6.003,17 em 12/01 é exatamente o total da fatura Nubank de janeiro. A busca cobre o mês do pagamento e os vizinhos, para cobrir pagamento adiantado ou em atraso, e lê cada par (cartão, período) uma vez só. Só vincula quando não há dúvida: dois cartões com o mesmo total no mesmo mês devolve nada e o usuário escolhe, porque vínculo errado liquida a fatura errada. Empate entre períodos do mesmo cartão desempata pelo vencimento mais próximo do pagamento. Pagamento parcial não casa com nada, de propósito.
- Importação de extrato: **transferência entre contas próprias é reconhecida sozinha**. Antes só existia uma regra com o CNPJ da Bravos Inter PJ fixo no código, e só para entrada. Agora a regra é geral e não depende de cadastro nenhum: quando a contraparte do Pix é o próprio titular do extrato — mesmo documento **e** mesmo nome do cabeçalho —, é dinheiro mudando de bolso, não receita nem despesa. Vale nos dois sentidos e para qualquer banco. A conta do outro lado é achada pela instituição citada na descrição ("MERCADO PAGO IP LTDA" → conta "Mercado Pago"); quando não dá para identificar, ou quando duas contas casam, a linha ainda entra como transferência e o app pede a conta, em vez de deixar passar como receita. No extrato real de janeiro: os R$ 6.000 vindos do Mercado Pago em 12/01 são reconhecidos, e nenhum dos 27 Pix para terceiros vira falso positivo. Sem isso, um Pix entre contas próprias inflava o resultado do mês dos dois lados.
- **Extrato de conta do Nubank em PDF.** Antes o arquivo era recusado com "PDF não reconhecido" — havia parser para fatura do Nubank e para extrato do Inter, mas não para este. O documento tem uma estrutura própria: não imprime sinal por lançamento, agrupa por dia e, dentro do dia, por direção ("Total de entradas + X", "Total de saídas − Y"), e **cada grupo declara o próprio subtotal**. O sinal de uma linha vem do grupo em que ela está. No topo há um bloco de saldos que também fecha (`inicial + rendimento + entradas − saídas = final`), então o extrato passa a trazer de graça a conferência que o OFX nunca teve. Verificado no extrato real de janeiro/2026: 29 lançamentos, entradas R$ 8.629,88 e saídas R$ 8.596,15 batendo com os totais declarados, os 18 subtotais diários fechando e resíduo R$ 0,00 no bloco de saldos.
- Importação de fatura Nubank: o **resumo do banco passa a ser lido como um livro que se fecha**. Toda fatura traz uma conciliação completa — `Fatura anterior − Pagamento recebido − Créditos + Encargos de financiamento + Compras + Outros = Total a pagar` — e ela fecha ao centavo. O parser lia 3 das 9 linhas desse bloco, nenhuma como identidade a conferir; agora lê todas, soma, e compara com o total declarado. Quando fecha, o total da fatura vem dele (fonte `PDF (resumo conferido)`) em vez da heurística que comparava cabeçalho com soma das linhas por razão de 2× — regra criada para um bug do Inter e que erra sempre que a fatura tem crédito, rotativo ou parcelamento, porque aí o total legitimamente não é a soma das linhas. Conferido nos cinco PDFs reais de jan, mai, jul (dois) e ago de 2026: todos com resíduo R$ 0,00. Três correções vêm de graça: o bloco é **delimitado** (antes as buscas varriam o documento e o `Total a pagar` do boleto podia vencer o do resumo), `Pagamento recebido` soma **todas** as ocorrências (antes só a primeira, então mês com dois pagamentos perdia um), e `Saldo financiado` é tratado como informativo — ele é derivado de `anterior − pagamento`, e somá-lo erraria junho em R$ 3.025,23.
- Conferência do total da fatura: link **Abrir arquivo** ao lado do tipo, abrindo em outra aba o arquivo que está sendo conferido. Decidir uma divergência exigia sair da tela para achar o PDF; agora ele abre ali. Sai do próprio arquivo já carregado no navegador — o mesmo que montou a revisão, tanto no upload quanto no reprocessamento —, sem volta ao servidor.
- Limites do cartão lidos da fatura. O bloco "Limites disponíveis" traz o limite total e a sua composição, então a revisão passa a mostrar o que a fatura declara contra o que está cadastrado — total e a parcela lastreada por investimento ("Nu Limite Garantido"), que é a que muda conforme aporte e resgate. Atualiza ao confirmar, nunca em silêncio: o limite pode ter sido ajustado à mão. Requer a migration `20260821120000_cartoes_limite_garantido.sql`.
- Pagamento parcial de fatura de cartão. Em cartão de crédito é possível pagar parte da fatura: o resto entra na fatura seguinte como "valor pendente do mês anterior", com juros e IOF do rotativo. O registro ficava duplamente errado — a fatura constava paga por inteiro e a conta era debitada por um valor que nunca saiu dela. Agora a fatura admite o status **Pago parcialmente** com o valor efetivamente pago, e ao importar a fatura seguinte a revisão mostra um bloco de conferência da anterior: total, quanto foi pago, quanto rolou, a correção do débito na conta e quanto dos pagamentos amortiza a fatura atual. O bloco é uma **conferência visual**: uma tag por ponto checado — valor, situação e data do pagamento — verde quando confere, âmbar mostrando os dois lados quando não. Na maioria dos meses a fatura anterior já está correta e nada é gravado; a caixa de confirmação só aparece quando a importação mudaria algo de fato. O bloco aparece em toda importação de fatura de cartão. No PDF do Nubank os números vêm do próprio arquivo, sem inferência: o bloco "Resumo da fatura atual" declara **Fatura anterior** e **Pagamento recebido**, e o carrego é a diferença entre os dois. A data sai da linha de pagamento no fim da fatura ("13 ABR Pagamento em 13 ABR"). Quando o arquivo não traz linha de pagamento — hoje o caso dos PDFs, cujo parser não extrai essa linha — ele mostra o que o arquivo diz por omissão (não há saldo pendente do mês anterior, logo nada ficou rolando), avisa que valor e data não podem ser conferidos, e não altera nada. Não só quando houve rotativo: todo arquivo carrega o pagamento da fatura passada, e comparar esse valor com o total cadastrado dela é uma validação de graça — divergência aponta lançamento faltando ou valor errado no mês anterior. Sem pagamento e sem carrego o arquivo não afirma nada, e nada é escrito. A confirmação fica no diálogo final da importação, marcada por padrão e podendo ser recusada — nada é reescrito em mês fechado sem esse aval. Requer as migrations `20260820010000_faturas_pagamento_parcial.sql` e `20260820020000_rpc_apply_invoice_import_liquidacao_anterior.sql`.

  O quanto foi pago é **deduzido dos números do banco**, não de heurística de data: o arquivo não diz a qual fatura cada pagamento foi aplicado, mas diz quanto NÃO foi pago. Então `pago = total da fatura anterior − valor pendente`, e o que sobra dos pagamentos amortiza a atual. Tentar deduzir por data de vencimento erraria: junho tem um pagamento no vencimento e uma amortização posterior, e o banco trata os dois de formas diferentes.
...
- Análise com IA: **modelo de reserva** em Ajustes → Inteligência artificial, recolhido atrás de um toggle. Quando o modelo principal falha por cota esgotada, indisponibilidade do provedor ou por não suportar a análise, o lote é repetido na reserva em vez de derrubar a análise inteira. A reserva aceita **chave própria** — inclusive uma segunda chave do mesmo provedor com o mesmo modelo, que é o que resolve cota semanal esgotada — guardada criptografada. Chave rejeitada não cai para a reserva, de propósito: erro de configuração precisa aparecer. Requer a migration `20260819130000_preferencias_ai_fallback.sql`; instância que ainda não migrou continua funcionando sem o recurso.
- Histórico de importações: **reprocessar** uma fatura já importada, sem tocar no lote original. O botão clona o lote no servidor (mesmo arquivo salvo, sem rascunho antigo) e abre uma revisão nova a partir dele — útil quando um erro manual posterior, ou uma correção como as desta versão, deixa a fatura desatualizada em relação ao arquivo.
- Detalhes do lançamento: parcelamento passa a listar **todas as parcelas da série** — número, mês da fatura e valor, com a parcela aberta destacada, check nas pagas, soma do que está cadastrado e aviso quando falta parcela. O valor fora do padrão da série aparece destacado; a referência é a moda dos valores, não a média, para que uma parcela errada não faça todas as outras parecerem divergentes. O botão **Alterar** de cada linha abre a edição daquela parcela sem sair do fluxo.
...
### Alterado
- Cards de **Receitas** e **Despesas** passam a mostrar duas leituras: o valor em destaque é o que **foi efetivado** no período, e logo abaixo vem o **previsto** do mês inteiro, com o quanto falta efetivar. O total sozinho dizia "entradas do período" para dinheiro que ainda não entrou — em setembro/2026 o card exibia R$ 33.000,00 num mês em que nada tinha entrado, porque somava a recorrência lançada, a compra em fatura aberta e o boleto a vencer. A comparação com o mês anterior passa a ser efetivado contra efetivado. A separação vem da mesma fonte, agrupada por `realizado` no RPC — e enquanto a migration não é aplicada o card continua se comportando como antes, com um número só, em vez de zerar. Requer a migration `20260902100000_rpc_period_overview_realizado.sql`, que também tira o ajuste de saldo desses cards.
- **Ajuste de saldo não conta como receita nem despesa nos relatórios.** Ele é andaime de conciliação — existe para impor a abertura que o extrato declara enquanto os meses anteriores não foram importados, e some quando eles entram —, mas entrava nos totais como movimento de verdade: importar agosto da conta Nubank sem os meses de trás produz um ajuste de R$ 39.905,96 que nunca entrou na conta, e ele caía inteiro em "Outras receitas". Agora fica fora do painel (receitas e despesas do mês, categorias, maiores gastos, estabelecimentos, formas e condições de pagamento) e dos totais por categoria dos relatórios. Continua visível no extrato da conta e na lista de lançamentos, que é onde ele precisa ser visto — e continua contando no **saldo**, que é a razão de ele existir. Casa pelo nome, e não por anotação, para valer também nos ajustes já gravados. Requer a migration `20260901220000_rpc_category_totals_sem_ajuste_de_saldo.sql`.
- Carrossel de meses da fatura: o ponto do gráfico passa a representar **o que foi pago**, não o total, no mês pago em parte — a linha desenha o dinheiro que se moveu, e plotar o total fazia maio subir como se tudo tivesse sido pago, justo o mês em que não foi. Os cards também ficam todos da mesma altura: a segunda linha do mês pago em parte esticava só o card dele.
- Fatura paga em parte: o valor em destaque passa a ser **o que foi pago**, com o total da fatura ao lado — "R$ 1.000,01 pagos de R$ 6.525,24" — no card e no carrossel de meses. O total sozinho fazia maio parecer paga por inteiro. Vale só na fatura paga em parte: no mês quitado o pago é o próprio total, e num mês futuro seria R$ 0,00, que leria como fatura sem valor. Com o destaque carregando o total pago, o bloco de pagamentos para de repetir o número e mostra só o pendente e a lista.
- Fatura do cartão: bloco de pagamentos com **total pago** e **pendente**. O valor pago vinha solto na linha de status, sem o contraponto do que ficou faltando — em maio dava para ler R$ 1.000,01 e não saber que R$ 5.525,23 rolaram. Agora os dois números ficam lado a lado, e abaixo deles a lista dos pagamentos: data, conta debitada e valor, um por linha. **Clicar em um pagamento abre o lançamento** no mesmo diálogo da tela de lançamentos, então corrigir valor, data ou conta reflete no extrato da conta corrente sem uma segunda viagem até lá. Com um pagamento só e nada pendente a lista aparece sozinha, sem repetir o total.
- Conferência do mês anterior: o número em destaque é o **total pago no mês**, e não a parte que abateu a fatura anterior — ver R$ 1.000,01 quando saíram R$ 3.500 da conta parecia erro. A divisão fica na lista logo abaixo: quando o arquivo traz mais de um pagamento, cada um aparece com data, valor e o que fez — "abateu Maio de 2026", "amortizou esta fatura" ou a divisão entre as duas. Um número só escondia que o pagamento foi feito em partes.
- Fatura paga: o **valor do pagamento** aparece junto da data na linha de status ("Pago em 12 de abr. de 2026 · R$ 10.430,50"). Quando o mês teve mais de um pagamento — antecipação para liberar limite, ou uma parte na data e o resto depois — eles são listados um por linha, com o total, porque só assim dá para conferir que a soma fecha com a fatura. A leitura passou a ser de lista: buscar só o primeiro escondia os demais.
- Fatura do cartão: bloco de conferência com o arquivo mais enxuto. Quando fecha, os três números ficam em uma linha e a explicação do arredondamento do banco some — ela é ruído quando não há divergência. Divergindo, o detalhe volta inteiro.
- Tabela de lançamentos: **sem rolagem lateral**. As onze colunas pediam mais de 2.000px e não cabiam em monitor nenhum — a tabela rolava para o lado em qualquer largura. Cada lançamento passa a ocupar dois andares: em cima estabelecimento, valor e ações, alinhados em colunas; embaixo, em linha, transação, condição, forma de pagamento, categoria, pessoa e conta. A descrição não perdeu espaço, que era o ponto.
- Confirmação da importação: "Excluídos do arquivo" passa a ser **"Serão ignorados do arquivo"**, e o filtro "Excluídos" vira "Ignorados". O arquivo nunca é alterado — essas linhas apenas não entram —, então "excluir do arquivo" descrevia algo que não acontece. O vermelho e o sinal de menos saem dessa linha e ficam só em "serão removidos do cadastro", que é a única que apaga lançamento.
- Confirmação da importação: as caixas de seleção do diálogo viraram **toggles**, que se leem de longe e deixam claro o que está ligado. Bloco da fatura anterior mais compacto, com os pontos conferidos em linha e ícone de check ou alerta em cada um, no lugar dos sinais em texto. E o bloco "esta fatura já está paga" ganha **Corrigir a data ou desfazer o pagamento**: marcando, a pergunta de pagamento volta, permitindo trocar a data ou marcar a fatura como em aberto.
- Reprocessamento de fatura: o diálogo de confirmação deixa de perguntar "esta fatura já foi paga?" quando ela já está paga. No lugar, informa a data e o valor do pagamento registrado e avisa que ele não será registrado de novo — reprocessar existe para aplicar melhorias sobre um arquivo já processado, e repetir a pergunta ignorava o que já tinha sido feito. A baixa fica bloqueada nesse caso, mesmo com o toggle ligado.
- Confirmação da importação: o diálogo passa a repetir, ao lado do botão que aplica, tudo o que será alterado fora dos lançamentos — ajuste da fatura anterior e atualização dos limites do cartão, cada mudança em uma linha, com a sua própria caixa de confirmação. O bloco da revisão fica longe do botão, e clicar em confirmar sem saber que o limite também muda era fácil. Esses ajustes também passam a contar como trabalho a confirmar. Reprocessar uma fatura já conferida e paga não mexe em lançamento nenhum, mas pode corrigir esses dois — e o botão de confirmar ficava desabilitado com trabalho pendente na tela.
- Importação: **modal de progresso** ao subir arquivo e ao reprocessar. Recuperar o arquivo, ler os lançamentos e conferir com o cadastro são três etapas assíncronas que mexiam no mesmo estado da tela — o passo do fluxo avançava e voltava conforme o resultado ia chegando, e a tela piscava. O modal cobre a transição e nomeia a etapa em andamento, que antes era invisível. Fica no mínimo meio segundo em tela, para que preparo rápido não vire outro piscar.
...
### Corrigido
- **Lançar receita, despesa ou transferência pela home não mexia em número nenhum.** A página é renderizada no servidor e os diálogos gravam por rota de API — nada no cliente sabia que os valores tinham mudado, então o lançamento recém-criado só aparecia na próxima navegação, e quem ficava na home via saldo e resumo do mês parados. As três ações rápidas agora pedem uma nova renderização ao salvar, e com ela vêm todos os números da página de uma vez: cards de receitas, despesas, balanço e previsto, saldo das contas, carrossel de meses e widgets. Conferido no ambiente real com uma despesa de R$ 0,01: despesas do mês, balanço, previsto e saldo total mudaram sem recarregar, e voltaram ao valor original quando o lançamento foi removido.
- **"Forma de pagamento" e "Conta" saíam desalinhados no diálogo de lançamento.** O campo com botão de adicionar (Conta, Cartão, Categoria) tinha cabeçalho mais alto que o campo vizinho sem botão, e os dois selects da linha ficavam em alturas diferentes. O cabeçalho passou a ter altura fixa igual à do botão, e virou um componente só — antes era o mesmo trecho duplicado em dois arquivos, o que também desalinhava "Tipo de transação" e "Categoria".
- **Filtrar por "com anexo", ou buscar sem resultado, derrubava a página de Lançamentos.** A ponte traduzia `sql\`false\`` — que é como o app diz "nenhum resultado" — para `id = null`. No PostgREST isso vira `id=eq.null`, que manda converter o **texto** `null` para `uuid` e volta `22P02 invalid input syntax`, ou seja, HTTP 400. Como a contagem usa `head: true` e resposta `HEAD` não tem corpo, o erro chegava vazio: o log dizia `error: Object`, o overlay dizia `{}`, e a página caía inteira no error boundary sem deixar pista. A condição impossível agora é atômica (`id.is.null` **e** `id.not.is.null`) e não depende do tipo de nenhuma coluna — diferente do sentinela `00000000-…` que a ponte usava para `in` vazio, que só funcionava em `uuid`.
- Ponte PostgREST: **nulo em comparação virava literal**. `eq`, `neq`, `gt`, `gte`, `lt` e `lte` com valor nulo emitiam `col=eq.null` e quebravam a consulta em qualquer coluna tipada; um `null` no meio de um `in` quebrava a lista inteira. Agora comparação com nulo não casa nada (como em SQL, onde `col = NULL` é `UNKNOWN`), o `in` descarta os nulos, e um aviso nomeia tabela, coluna e operador — foi esse aviso que apontou o `id`/`eq` e fechou o caso.
- Ponte PostgREST: **um ramo de `or` com vírgula descartava o `or` inteiro**. A checagem de "todos os ramos traduziram" contava vírgulas na string montada, então um `in.(a,b)` chunkado — ou a nova condição impossível — fazia a conta não bater e o filtro sumia em silêncio, **alargando** o resultado. A verificação passa a ser por ramo, não por vírgula.
- Ponte PostgREST: **falha de contagem era invisível por construção**. A contagem usa `head: true`, que manda um `HEAD` — e resposta `HEAD` não tem corpo. Quando o PostgREST recusava a consulta, o supabase-js não tinha JSON de erro para parsear e devolvia um objeto vazio: o log dizia `error: Object`, o overlay dizia `{}` e a exceção dizia "Falha na consulta PostgREST", enquanto a página de Lançamentos caía inteira no error boundary. Agora, quando o erro chega vazio, a consulta é relida uma vez em `GET` **só para recuperar a explicação** que o `HEAD` engoliu, e essa explicação vai para o log e para a mensagem da exceção — que é o que o overlay mostra. O status HTTP também passa a ser registrado; foi ele que revelou que o caso real era um **400**, não queda de rede.
- Ponte PostgREST: **4xx deixou de ser retentado**. Como um 400 chegava sem corpo, ele era classificado como falha de transporte e repetido três vezes — sempre com o mesmo 400, só atrasando a página. Repetição fica para 5xx, 429 e erro sem status (aí sim, transporte).
- Ponte PostgREST: **rajada de consultas paralelas estourava o TLS**. Medido contra o Supabase real: até 10 conexões novas simultâneas passam sempre, a partir de 20 começam a falhar handshakes — e o `fetch` do Node devolve o mesmo erro sem corpo. O cliente agora enfileira em no máximo 8 requisições em voo, o que mantém o pico baixo e deixa o undici reaproveitar conexão em vez de refazer o handshake a cada consulta.
- Ponte PostgREST: **erro sem mensagem era reportado como falha de consulta**. O supabase-js devolve um objeto vazio quando o `fetch` falha ou a resposta não é o JSON de erro esperado — sem `message`, `details`, `hint` nem `code`. O log imprimia `error: Object` e a exceção dizia "Falha na consulta PostgREST", as duas mandando quem investiga procurar SQL errado quando o problema é rede. Aconteceu na página de Lançamentos, que caiu inteira no error boundary sem deixar pista. Agora o log achata os campos do erro (as chaves do PostgrestError não são enumeráveis, por isso apareciam como `Object`) e a mensagem diz que é falha de transporte.
- Importação de extrato: **lançamento de outra conta era dado como duplicata**. A busca de candidatos traz de propósito as pernas de transferência que vivem em outras contas — é delas que sai o mapa de conta-par —, mas elas também entravam na conferência de duplicata. No extrato Inter de 01/09 havia uma aplicação em CDB de −R$ 200,00 que casou com a perna de −R$ 200,00 do Mercado Pago, foi marcada como "Conferido" e nunca era importada; o saldo ainda assim fechava, porque a perna de entrada de R$ 200,00 acabava removida na conciliação e os dois erros se cancelavam. Agora só lançamento da conta que está sendo importada (ou sem conta) é candidato a duplicata.
- Importação de extrato: **a direção da transferência era ignorada ao casar duplicata**. Como "Transferência" não mapeia para receita/despesa, a comparação de valores desistia e aceitava qualquer sinal — então uma saída do arquivo casava com uma entrada do cadastro de mesmo dia e mesmo valor. O sinal do valor guardado passa a decidir a direção.
- Conferência de saldo: **perna de transferência sem par nenhum não era reconhecida como órfã**. A regra exigia par em outra conta, então a perna cujo outro lado foi apagado caía no balde genérico de "lançamentos que não estão no extrato" — que o app só sabe apontar, em vez de remover na confirmação como faz com as demais órfãs.
- Corrigido erro de tipo em `statement-balance-reconciliation.ts` que quebrava `tsc --noEmit` na CI (`Boolean(...)` não estreita `string | undefined`).
- Conferência de saldo do extrato: **pagamento de fatura não contava no líquido da conta**. O dinheiro sai da conta corrente como qualquer despesa e o extrato o registra como registra as demais, mas o cálculo o excluía — no extrato Inter de agosto/2026 o débito automático de R$ 78,00 sumia e o líquido do arquivo dava −R$ 938,99 contra os −R$ 1.016,99 que os próprios saldos declaram (1.017,81 − 1.016,99 = 0,82, o saldo final). Transferência tinha o mesmo problema do lado das linhas novas: os dois lados da conferência usavam filtros diferentes e mediam coisas diferentes. Agora ambos usam `isAccountStatementMovementImportRow`.
- Importação de extrato: **a perna de transferência já cadastrada não era reconhecida** e o mesmo dinheiro entrava duas vezes. Ao reconhecer uma transferência o app substitui a descrição do extrato por "Saída/Entrada - Transf. entre contas"; quando o extrato da conta do outro lado era importado depois, a mesma movimentação chegava com o nome do banco, não casava com nada e um par novo nascia — cada extrato criando o seu. No Inter de agosto isso duplicou R$ 1.410,00 em três transferências. A perna passa a casar por data e valor, que são os campos que o banco declara e o app não reescreve; divergir do nome genérico deixa de ser divergência, porque foi o próprio app que o escreveu.
- Importação de extrato de conta: o **período do arquivo era carimbado em toda linha**. Fatura de cartão tem um período — toda compra do arquivo pertence à fatura importada —, mas extrato de conta não: o recorte é arbitrário (uma semana, um mês, um ano) e cada lançamento pertence ao mês da sua própria data. Como a tela abre a importação com um período na URL e o caminho de conta nunca o recalculava, um extrato que atravessa meses caía inteiro num mês só, e o seletor de período fica escondido para conta, então nem dava para corrigir à mão. Na conta Nubank isso arquivou 65 lançamentos de julho no mês de agosto: o líquido de agosto no cadastro dava +R$ 2.563,11 contra os −R$ 1.206,12 que o extrato declara. Agora o período sai de `resolveImportRowPeriod` nos três pontos que o derivavam (linha comum, recorrência e transferência).
- Pagamento de fatura com o nome do extrato anexado à anotação deixava de contar como pago. A importação grava a anotação em duas linhas — `AUTO_FATURA:<cartão>:<período>` e, embaixo, `Extrato: <nome original>` —, e a leitura por `split(":")` fazia o período sair como `"2026-01\nExtrato"` e ser descartado. O pagamento existia no extrato da conta, mas a fatura o ignorava: o carrossel de meses contava R$ 0,00 de pago em janeiro/2026, apesar dos R$ 6.003,17 que saíram da conta, e o mesmo acontecia com os R$ 78,00 do cartão Inter em agosto. A leitura passa a ser por padrão ancorado no início, tolerante ao que venha depois — inclusive à marca de amortização.
- Importação de extrato: **pagamento de fatura do cartão entrava como despesa comum**. O detector não conhecia o texto que o Nubank usa — "Pagamento de fatura", com o "de" que os padrões antigos (`pgto fatura`, `pagamento fatura`) não previam —, então a linha não virava pagamento de fatura e não liquidava fatura nenhuma. No extrato de janeiro é o lançamento de R$ 6.003,17 em 12/01, exatamente o total da fatura Nubank do mês. O padrão é ancorado no início, para não casar com "Estorno de pagamento de fatura" nem com menção no meio da frase.
- Extrato Nubank em PDF: o **cabeçalho de página grudava na descrição** do lançamento seguinte. Nome do titular, CPF, agência e conta abrem cada página no meio das movimentações, e só parte disso era removida — sobrava "Franklin ... 472152010-3" como prefixo do lançamento.
- Revisão da importação: a **descrição deixa de ficar escondida**. A coluna era travada em 200px e o campo era um input de uma linha, enquanto os selects de categoria e tipo cresciam à vontade — num extrato do Nubank, onde a descrição passa de 130 caracteres, todas as linhas apareciam como "Transferência" e era impossível distinguir uma da outra. Agora a descrição tem a largura garantida, os selects respeitam o tamanho do cabeçalho, e o campo cresce com o texto em vez de cortar. Verificado no extrato real de janeiro: nenhuma das 29 descrições cortada.
- Leitura de valores: o **`+` explícito** era recusado. `parseBrazilianAmountOrNull` tratava `-`, parênteses e menos Unicode, mas não o mais que o extrato usa em "Total de entradas +8.629,88" — a string inteira era invalidada e o valor virava nulo. Apareceu ao ler o bloco de saldos do extrato Nubank, onde fazia o total de entradas sumir.
- Liquidação da fatura anterior: **diferença de cadastro deixava de virar pagamento fantasma**. O quanto foi pago sai de `pago = total da fatura anterior − carrego`, e o total usado era o **cadastrado** — então qualquer desvio de registro do mês passado saía dessa subtração como se fosse dinheiro pago. Julho/2026 é o caso: o arquivo de agosto declara carrego de R$ 2.109,50, exatamente o total que o banco declarou para julho, ou seja a fatura rolou inteira e ninguém pagou nada. Como o cadastro estava R$ 41,90 mais alto, apareceu um pagamento de R$ 41,90 que nunca existiu, julho virou "paga parcialmente" e o valor em destaque na tela passou a ser a própria diferença. Agora a conta usa o total que o **banco** declarou no arquivo daquela fatura, e a divergência entre declarado e cadastrado aparece como ponto de conferência em vez de desaparecer dentro de um pagamento.
- Importação: **data impossível e valor ilegível não passam mais em silêncio**. Os parsers montavam `YYYY-MM-DD` por concatenação, então `31/02/2026` virava `"2026-02-31"`, `parseLocalDateString` devolvia `Date(NaN)`, o período derivado saía `"NaN-NaN"` e a data inválida ia para uma coluna `date not null`. As cinco funções de data passam a validar o calendário pelo mesmo teste que o resto do app já usa. E o valor: `parseBrazilianAmount` devolvia `0` no ilegível, o que fazia a linha ser descartada pelos parsers (`if (amount === 0) continue`) — extrato com parênteses negativos `(1.234,56)`, menos à direita `1.234,56-` ou menos Unicode `−` perdia lançamento sem deixar rastro. Agora existe `parseBrazilianAmountOrNull`, que distingue "zero" de "não consegui ler", reconhece as três formas de negativo e escolhe o separador decimal pelo último `,` ou `.` — o que faz `1.234,56` e `1,234.56` lerem certo.
- Fatura entrando maior que o valor cobrado pelo banco. A linha "valor pendente do mês anterior" é apurada na data de vencimento da fatura passada; um pagamento que chega depois reduz o saldo financiado e o banco **não emite linha de crédito** por ele no extrato — apenas abate do total a pagar. O OpenMonetis gravava a linha vencida, e a fatura ficava maior. Em junho/2026 o OFX declara carrego de R$ 5.525,23 (apurado em 12/05) e total de R$ 7.978,14, enquanto suas próprias linhas somam R$ 10.478,14: os R$ 2.500,00 de diferença são o pagamento de 18/05, e o resumo do banco confirma o saldo financiado de R$ 3.025,23. Agora a importação corrige o valor da linha pelo total que o banco manda pagar — e refaz o id sintético junto, que embute o valor. A diferença só é atribuída ao carrego quando ele existe e a cobre; fora disso, sobra continua sendo sobra, porque mascarar isso esconderia lançamento faltando.
- Pagamento antecipado da fatura: **o dinheiro saía da conta e não havia lançamento nenhum**. Quem paga em vários dias para reduzir juros abate a fatura em formação antes de ela vencer, e só o arquivo do mês seguinte revela esse abate. A importação o deduzia e mostrava na revisão ("amortizou esta fatura") — e não gravava: o extrato dizia que R$ 10.478,14 saíram no vencimento de junho, quando R$ 2.500,00 saíram em 18/05 e R$ 7.978,14 em 12/06. Agora a importação registra cada abate como pagamento da fatura, na data em que o dinheiro saiu, com toggle próprio no diálogo de confirmação. A baixa da fatura passa a descontar o que já está registrado, senão os dois débitos somariam mais do que saiu da conta. O período do lançamento é o mês da saída, não o da fatura: o extrato agrupa por período, e no mês da fatura a saída de maio só apareceria em junho — que é o problema que o registro resolve.
- Distribuição dos pagamentos: o centavo de arredondamento do banco vazava para o pagamento seguinte. Em junho, `pago = 6.525,24 − 5.525,23` dá R$ 1.000,01 contra uma linha de R$ 1.000,00, e o centavo que faltava era buscado no pagamento de R$ 2.500,00 — que passava a valer R$ 2.499,99. Invisível enquanto era só exibição; com o valor sendo gravado, era um erro de um centavo no extrato.
- Fatura paga parcialmente na tela do cartão. A importação gravava o status `parcial`, o valor pago e a data — e a página da fatura ignorava os três: a leitura só reconhecia "pendente" e "pago", então maio aparecia **Em aberto**, sem valor e sem data, enquanto cada um dos 81 lançamentos dela exibia o check verde de "Fatura paga". A mesma tela afirmava as duas coisas. Agora a fatura mostra **Pago parcialmente**, o que foi pago e quando, e uma linha com o que rolou para a fatura seguinte. No carrossel de meses ela deixa de aparecer em vermelho como vencida — não está: parte foi paga e o resto virou cobrança do mês seguinte. E o check de cada lançamento vira âmbar, com "Fatura paga parcialmente": o lançamento continua liquidado (o que não foi pago não fica pendente, é recobrado no mês seguinte), mas dizer "Fatura paga" contradizia o cabeçalho logo acima.
- Conferência do mês anterior: a data era comparada com a do pagamento **mais recente** do arquivo. Quem paga em vários dias para reduzir juros tem, no mês seguinte, um pagamento que liquidou a fatura passada e outros que amortizam a atual — o mais recente é o segundo caso, e a conferência acusava divergência onde não havia. Agora os pagamentos são distribuídos em ordem cronológica e a data conferida é a do que de fato abateu a fatura anterior.
- Importação: reprocessar uma fatura já conferida devolvia "Selecione ao menos uma transação" quando não havia lançamento a importar. A validação do servidor não contava a liquidação da fatura anterior como trabalho, e o cliente nem chamava a ação quando só os limites mudavam — o botão habilitava e a importação recusava.
- Liquidação da fatura anterior: a **data do pagamento** passa a ser corrigida junto com status e valor. A conferência apontava a divergência de data, mas confirmar não a resolvia — o usuário via um problema sinalizado e sem saída dentro do fluxo. O diálogo agora lista cada mudança em uma linha ("o débito vai de X para Y", "a data vai de A para B"), em vez de uma frase só que falava apenas do valor.
- Importação de fatura: identificador que o arquivo repete em cobranças diferentes. O Nubank emite um FITID só para todo o bloco do rotativo — "valor pendente do mês anterior", juros e IOF chegam com o id idêntico, e o mesmo id volta na fatura seguinte. As três linhas casavam com o único lançamento que conseguira guardar esse id, saíam da conferência como "já cadastrado em outro período" e abriam um furo do tamanho delas (R$ 6.135,89 na fatura de junho); o índice único ainda deixava entrar só uma. Quando o mesmo id aparece em linhas de conteúdo diferente ele é mais grosseiro que a cobrança e passa a ser trocado por um id derivado da própria linha — único por cobrança e estável entre reimportações. Id repetido em linhas de conteúdo igual continua tratado como antes.
- Importação de fatura: fatura de rotativo não fechava. O total declarado pelo banco é o **saldo**, já líquido do que foi pago no ciclo, enquanto os lançamentos são as cobranças brutas — e quanto de cada pagamento o banco já embutiu no "valor pendente" não está no arquivo. Quando o arquivo declara pagamentos recebidos, a conferência passa a usar a soma das cobranças do próprio arquivo, que é também o total da fatura no OpenMonetis, e o banner explica o porquê em vez de mostrar uma diferença sem justificativa.
- Importação de fatura: duas cobranças idênticas no mesmo arquivo — dois pedágios no mesmo dia pelo mesmo valor, por exemplo — tinham a segunda colapsada como repetição, abrindo no total projetado um furo exatamente do valor dela. Em PDF/CSV o identificador é sintético (`data|descrição|valor`), então linhas idênticas colidem por natureza e ganham sufixo `#2`: é ele que distingue uma cobrança da outra, e a comparação passa a levá-lo em conta. Identificador vindo do arquivo (FITID) repetido continua tratado como repetição do parser, que é o que realmente é.
- Consultas via Supabase: a ponte do Drizzle **ignorava a ordenação** e, mesmo depois de aplicá-la, lia `desc()` como `asc()` — o `" desc"` fica em `value` como array de strings e o extrator só olhava strings simples. Todo `orderBy` do app vinha silenciosamente invertido ou fora de ordem; na prática, a página da fatura lia o lote de importação mais antigo em vez do mais recente e nunca mostrava a conferência.
- Consultas via Supabase: o erro do banco perdia o SQLSTATE ao ser reembrulhado, então violação de índice único (23505) caía na mensagem genérica em vez da mensagem específica. O código passa a ser preservado no objeto de erro.
- Importação de fatura: a parcela do mês corrente era descartada sempre que qualquer outra parcela da mesma compra já estivesse gravada. O Nubank repete o mesmo identificador (FITID) em toda a série — ele identifica a **compra**, não a cobrança do mês — e o filtro de inserção tratava isso como duplicata. A fatura ficava eternamente sem aquela linha e reprocessar não resolvia. Agora a ocorrência entra sem o identificador, que fica com a linha que já o possui, respeitando o índice único `(user_id, ofx_fit_id)`.
- Importação de fatura: reprocessar uma fatura recriava as parcelas futuras por cima das que já existiam, acumulando uma cópia por reprocessamento em faturas já fechadas.
- Importação de fatura: a parcela que faltava entrava numa série nova, solta das irmãs — cada metade da compra virava um parcelamento separado na tela. A importação passa a reaproveitar a série já gravada, identificada por estabelecimento, total de parcelas e mês da primeira parcela.
- Importação de fatura: data de compra das parcelas. O Nubank carimba em cada parcela a data de abertura do ciclo; outros cartões repetem a data original da compra. Importar primeiro uma fatura do meio da série fazia a série inteira herdar uma data **posterior à própria fatura** em que aparece. A data do ciclo passa a ser trazida de volta para o mês da primeira parcela, e a série adota sempre a data mais antiga entre as candidatas — a compra não acontece depois do ciclo que a cobra.
- Importação: a memória de categoria por descrição não funcionava. A consulta montava a chave com uma expressão SQL crua que o PostgREST não expressa: o filtro era descartado em silêncio e a coluna calculada não voltava, então o histórico de lançamentos nunca era consultado e toda linha caía para a IA, mesmo com a categoria já preenchida à mão antes. A normalização e o casamento por prefixo passam para a RPC `get_import_description_memory`. Requer a migration `20260819230000_rpc_import_description_memory.sql`; instância que ainda não migrou continua funcionando pelo caminho antigo.
...
- Importação de fatura: FITID reaproveitado entre as parcelas de uma mesma compra parcelada (comum no Nubank) deixa de ser tratado como reimportação. Cada parcela do arquivo era comparada ao mesmo identificador, e a primeira parcela cadastrada com esse FITID em qualquer mês travava a importação das demais como "já cadastrado em outro período" — mesmo quando aquele mês nunca tinha sido importado.
- Importação de fatura: duas cobranças reais no mesmo dia, pelo mesmo valor e com a mesma descrição (recarga de pedágio, por exemplo) deixam de ser colapsadas em uma só. A verificação de repetição dentro do próprio arquivo comparava apenas data, valor e descrição; agora, quando cada linha tem seu próprio identificador do banco, duas linhas diferentes contam como duas cobranças.
- Conferência da fatura (tela do cartão): lançamento parcelado deixa de aparecer como "a mais no OpenMonetis" e "ausente do arquivo importado". A comparação usava o nome cru — "Fabio C Thomaziello" cadastrado nunca batia com "Fabio C Thomaziello - Parcela 6/10" do arquivo — e toda parcela cadastrada aparecia errada nessa conferência, mesmo estando corretamente importada.
